---
title: "QSAR model by a LASSO-MLR regression"
...
In this case, LASSO L1 regularization will be used as feature selection technique and after that, the selected descriptors will be fitted in a classical multiple linear regression (MLR)</p>
...
## Fitting an ordinary least square multiple linear regression model with the selected descriptors by LASSO

<p style='text-align: justify;'>Now, if the two descriptors are selected, it is possible to fit a OLS-MLR model with a system of 20 observations (training set molecules) and 2 regressors (the two molecular descriptors selected by LASSO). This fit is straightforward and the only assumption is that the square errors from the fitting are normally distributed.
</p>

```{r warning=FALSE, message=FALSE}
library(dplyr)

##Selecting only the two descriptors generated by LASSO algorithm
Modelamiento_Lineal<-as.data.frame(Modelamiento)
Modelamiento_Lineal<-select(Modelamiento_Lineal,F04.C.S.,B10.C.S.,pIC50)

Prueba_Lineal<-as.data.frame(Prueba)
Prueba_Lineal<-select(Prueba_Lineal,F04.C.S.,B10.C.S.,pIC50)

##Fitting the training set into a OLS-MLR:
Lineal<-lm(pIC50~.,data=Modelamiento_Lineal)
summary(Lineal)
```
...
library(car)
qqPlot(Lineal, main="QQ Plot")
```
```{r}
##Confidence intervals for each coefficient

confint(Lineal,level = 0.95)
```
### Predicting the training dataset with the model (Internal):

```{r}

Predicho_tr<-predict(Lineal,newdata = Modelamiento_Lineal)

Observado_tr<-Modelamiento_Lineal$pIC50

```
### Predicting the test dataset with the model (External):

```{r}
Predicho_out <- predict(Lineal,newdata = Prueba_Lineal)

Observado_out<-Prueba_Lineal$pIC50
```
...
Modelamiento_CV<-as.data.frame(Modelamiento_Lineal)
set.seed(100000)
...
Modelo_Lineal_CV<-lm(pIC50~.,data=Calibracion_CV)

##Predicting the i-row

Pred_CV_model = predict(Modelo_Lineal_CV, newdata = Prueba_CV)

##Predicted values for each i-row
Pred_CV[i]<-Pred_CV_model
      
##Model for each i-row
Modelo_i[[i]]<-Modelo_Lineal_CV
...
ggsave("lassomlrloo.png")


```

### Y-randomization test for LASSO-MLR model
...
Modelado_shuffle<-select(Modelado_shuffle,F04.C.S.,B10.C.S.,pIC50)
Prueba_shuffle<-select(Prueba_shuffle,F04.C.S.,B10.C.S.,pIC50)
...
hi<-hatvalues(Lineal) ##Leverages

##Getting standardized residuals from the model

Stand_Residuales<-rstandard(Lineal) 
...
X_AD<-as.matrix(Modelamiento_Lineal)
X_test<-as.matrix(Prueba_Lineal)
...
obs_test<-Prueba_Lineal$pIC50
pred_test<- predict(Lineal,newdata = Prueba_Lineal)
...
geom_point()+ xlim(0,1) +geom_text(label=rownames(ggplot_William),check_overlap = TRUE,hjust = 1,vjust = 1)+ geom_vline(xintercept=0.45, linetype="dotted",color="red") + geom_hline(yintercept=c(-3,3), linetype="dotted",color="blue")+ggtitle("Williams plot (Applicability domain of LASSO regression")+ylab("Standardized residuals")
ggsave("lassomlrad.png")
...
Error<-as.numeric(c(0.129616,0.236944,0.244262))
CImin<-as.numeric(c(-0.005715959,-0.146371073,-0.281233845))
CImax<-as.numeric(c(0.9941014,0.8843239,0.2656958 ))
Coefficient<-as.numeric(c(0.494193,0.368976,-0.007769))
...
```{r warning=FALSE, message=FALSE}
Predicho_out_VS <- predict(Lineal,newdata = as.data.frame(VS_db_S))
...
write.csv(Predicho_out_VS[order(Predicho_out_VS$pIC50,decreasing = TRUE),],file = "VS_MLRLASSO-2.csv", row.names = FALSE)
...
```{r warning=FALSE, message=FALSE}
Predicho_out_VS3 <-  predict(Lineal,newdata = as.data.frame(VS_db3_S))
...
write.csv(Predicho_out_VS3[order(Predicho_out_VS3$pIC50,decreasing = TRUE),],file = "VS_MLRLASSO-3.csv", row.names = FALSE)
